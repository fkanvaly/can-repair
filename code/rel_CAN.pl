...
:- consult('rel_util0.pl').	% chargement d'utilitaires
%%%  Alternative:   :- consult('rel_util.pl').	% chargement d'utilitaires si le systeme graphique est present

:- tl(2).	% Sets trace level. Defined in 'rel_util0', but not in 'rel_util' where trace level is determined from within a window
